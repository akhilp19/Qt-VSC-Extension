...
import { getXcodebuildSigningArgs, buildExportOptionsPlist } from './qtIOSSigning';
...
export class QtIOSDeployment {
        ...
        await vscode.window.withProgress({
            ...
            return new Promise<void>((resolve, reject) => {
                ...
                if (isCMake) {
                    ...
                    if (buildForDevice) {
                        const xcodebuild = this.findTool('xcodebuild') || 'xcodebuild';
                        const xcodeProj = this.findXcodeProject(iosBuildDir);
                        if (!xcodeProj) {
                            reject(new Error('No Xcode project generated by qmake. Device builds require an .xcodeproj.'));
                            return;
                        }
                        if (!getXcodebuildSigningArgs()) {
                            void vscode.window.showWarningMessage('No iOS signing identity/profile configured. Device build may fail.');
                        }
                        const xcodeConfig = buildType === 'release' ? 'Release' : 'Debug';
                        const signingArgs = getXcodebuildSigningArgs();
                        commands.push(
                            `"${xcodebuild}" -project "${xcodeProj}" -destination 'generic/platform=iOS' -configuration ${xcodeConfig} build ${signingArgs}`
                        );
                    } else {
                        commands.push('make');
                    }
    ...
    async archiveIOSApp(projectFile?: string): Promise<void> {
        ...
        await vscode.window.withProgress({
            ...
            return new Promise<void>((resolve, reject) => {
                ...
                const signingArgs = getXcodebuildSigningArgs();
                const cmd = `"${xcodebuild}" archive -project "${targetProject}" -scheme "${scheme}" -archivePath "${archivePath}" -destination 'generic/platform=iOS' ${signingArgs}`;
    ...
    async exportIOSIpa(projectFile?: string): Promise<void> {
        ...
        // Generate ExportOptions.plist from configured signing/profile
        const config = vscode.workspace.getConfiguration('qt');
        const method = config.get<string>('iosExportMethod') || 'development';
        const profileUuid = config.get<string>('iosProvisioningProfile') || '';
        const teamId = config.get<string>('iosTeamId') || '';
        const appBundle = this.findAppBundle(archivePath);
        const bundleId = appBundle ? this.extractBundleId(appBundle) : undefined;
        const plistContent = buildExportOptionsPlist(method, profileUuid, teamId, bundleId || 'com.example.app');
    ...
    private findXcodeProject(buildDir: string): string | undefined {
        try {
